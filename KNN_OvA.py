...
one_vs_all_knn = OneVsRestClassifier(knn)
...
for fold, (train_indices, val_indices) in enumerate(zip(train_folds, validation_folds)):
    ...
    one_vs_all_knn.fit(X_train, y_train)
    val_pred = one_vs_all_knn.predict(X_val)
    ...
    test_pred = one_vs_all_knn.predict(X_test)
...
values = [np.mean(test_accuracies), np.mean(test_f1_scores), precision, recall]
